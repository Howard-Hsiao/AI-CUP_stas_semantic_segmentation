...
def eiou_loss(pred, target, eps=1e-7):
    ...
    eious = 1 - ious + (rho2 / c2) + (rho_w2 / cw2) + (rho_h2 / ch2)
    return eious
...
def focal_eiou_loss(pred, target, gamma=0.5, eps=1e-7):
    ...
    eious = 1 - ious + (rho2 / c2) + (rho_w2 / cw2) + (rho_h2 / ch2)
    loss = (ious ** gamma) * eious
    return loss


@mmcv.jit(derivate=True, coderize=True)
@weighted_loss
def focal_ciou_loss(pred, target, gamma=0.5, eps=1e-7):
    r"""`Implementation of paper `Enhancing Geometric Factors into
    Model Learning and Inference for Object Detection and Instance
    Segmentation <https://arxiv.org/abs/2005.03572>`_.
    ...
    # CIoU
    cious = 1 - ious + (rho2 / c2 + v ** 2 / (1 - ious + v))
    loss = ((1 - ious) ** gamma) * cious
    return loss
...
class FocalCIoULoss(nn.Module):
    ...
    def __init__(self, eps=1e-6, reduction='mean', loss_weight=1.0, gamma=0.5):
    ...
    def forward(self,
        ...
        if weight is not None and weight.dim() > 1:
            ...
            assert weight.shape == pred.shape
            weight = weight.mean(-1)
        loss = self.loss_weight * focal_ciou_loss(
            pred,
            target,
            weight,
            eps=self.eps,
